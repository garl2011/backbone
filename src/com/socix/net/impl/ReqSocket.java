...
public class ReqSocket extends Thread implements ISocket {
	private static Logger log = LogManager.getLogger(ReqSocket.class.getName());
	...
	public ReqSocket(Socket socket) {
		this.socket = socket;
		try {
			this.is = this.socket.getInputStream();
			this.os = this.socket.getOutputStream();
		} catch(IOException e) {
			log.error("ReqSocket:", e);
		}
					...
					if(FormatConverter.byte2int(head, 0) == 589505572) {
						...
						if(FormatConverter.byte2int(body, length) == random && FormatConverter.byte2int(body, length+4) == 606348067) {
							for(int i = 0; i < this.listener.size(); i++)
								this.listener.get(i).reply(body, 0, length);
						}
					}
				}
			}
