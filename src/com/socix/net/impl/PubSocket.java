...
public class PubSocket extends Thread implements ISocket {
	private static Logger log = LogManager.getLogger(PubSocket.class.getName());
	...
	public PubSocket(Socket socket) {
		this.socket = socket;
		try {
			this.is = this.socket.getInputStream();
			this.os = this.socket.getOutputStream();
		} catch(IOException e) {
			log.error("PubSocket:", e);
		}
					...
					if(FormatConverter.byte2int(head, 0) == 589505572) {
						...
						if(FormatConverter.byte2int(body, length) == random && FormatConverter.byte2int(body, length+4) == 606348067) {
							for(int i = 0; i < this.listener.size(); i++)
								this.listener.get(i).subscribe(body, 0, length);
						}
					}
				}
			}
	...
	public int writeData(byte[] data, int offset, int length) {
		if(enable) {
			...
			try {
				...
				os.write(rbody);
				os.flush();
				return rhead.length + rhead.length;
			} catch(IOException e) {
				log.error("PubSocket:", e);
				enable = false;
				return -1;
